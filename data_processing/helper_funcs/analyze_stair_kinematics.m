function [processed_struct] = analyze_stair_kinematics(foot_angle_data_unproc,shank_angle_data_unproc, ...
    heel_pos_forward_unproc, heel_pos_upward_unproc,...
    HS_frames,...
    treadmill_speed, treadmill_incline, stair_height, stride_count_start, stair_events, leg_side_num, SAMPLE_RATE_VICON)
%         only look at events starting at the first HS and ending at the
        %last HS
    HS_range_idxs = HS_frames(1):HS_frames(end);
    
    %handle if the last HS marker is an index greater than the range of the
    %data
    while HS_frames(end) > length(foot_angle_data_unproc)
        HS_range_idxs = HS_frames(1):HS_frames(end-1);
        HS_frames(end) = [];
    end
    ...
    %set up subject speed
    speed = treadmill_speed*ones(size(phase));
...
%         iterate through strides
    ...
    % generate is_stairs vector
    is_stairs = zeros(size(foot_angle_data));
    [rows, ~] = size(stair_events);

    for jj = 1:length(stair_events)
        %unpack heel strikes
        HS_idxs = stair_events{jj,2} - HS_range_idxs(1)+1;

        %only proceed if the HS_idx is within the range of the data
        if HS_idxs(2) <= length(foot_angle_data)
            activity_str = stair_events{jj,1};
    
            %classify stairs based on activity_str
            % if we're already on stairs, set is_stairs to 1
            if strcmp(activity_str,'s1') || ...
                    strcmp(activity_str,'s2') || ...
                    strcmp(activity_str,'s3')
                is_stairs(HS_idxs(1):HS_idxs(2)) = stair_height;
            
            % if we're transitioning to stairs from walk, linearly ramp up is_stairs from
            % 0 to 1
            elseif strcmp(activity_str,'w2s')
%                 is_stairs(HS_idxs(1):HS_idxs(2)) = linspace(0,1,length(HS_idxs(1):HS_idxs(2)));
                is_stairs(HS_idxs(1):HS_idxs(2)) = stair_height;

            % if we're transitioning to walk from stairs, linearly ramp down is_stairs from
            % 1 to 0
            elseif strcmp(activity_str,'s2w')
%                 is_stairs(HS_idxs(1):HS_idxs(2)) = linspace(1,0,length(HS_idxs(1):HS_idxs(2)));
                is_stairs(HS_idxs(1):HS_idxs(2)) = stair_height;
            end
        end
    end
    ...
    processed_struct.is_stairs = is_stairs;
end