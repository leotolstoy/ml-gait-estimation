from utils.attitudeEstimatorEKFFuncs import *
from time import time


class AttitudeEKF():
    """This class contains the implementation for an Attitude EKF
        that measures the spatial orientation of the shank using the 
        sensors onboard the Dephy ExoBoot
    """    
    ...
    def step(self, i, dt, isUpdateTime=True):
        ...
        Args:
            ...
            dt (float): time step
            isUpdateTime (bool, optional): whether to update using the measurement. Defaults to True.
        """        
        time0 = time()
        first=(i==0)
    ...
    def measure(self, i, gyroVec_corrected, accelVec_corrected, isUpdateTime=True, CORRECT_VICON=True):
        """Perform the measurement step of the EKF

        Args:
            i (_type_): _description_
            gyroVec_corrected (_type_): _description_
            accelVec_corrected (_type_): _description_
            isUpdateTime (bool, optional): _description_. Defaults to True.
            CORRECT_VICON (bool, optional): _description_. Defaults to True.
        """        
        time0 = time()
